class CalculatorError(Exception):
    """Raised when a calculation cannot be performed for logical/mathematical
    reasons."""
...
class HoldenRowlandCalculator:
    ...
    def clean_expression(self, expression_to_clean: str) -> str:
        """Removes all white space from the expression in case a space or tab was
        entered.
    ...
    def get_num_operators(self, expression: str) -> int:
        """Gets the number of operations that are to be done in the expression. If the
        expression is from the inside of parenthesis it gets only the number of
        operations in side the parenthesis.
        ...
        for i, char in enumerate(expression):
            if char in self._OPERATORS:
                if not (char == "-" and i == 0) and not (
                    char == "-" and ((expression[i - 1] in self._OPERATORS))
                ):  # skip negative numbers - don't count them as
                    num_operators += 1

        return num_operators

    def find_operator_pos(self, expr: str, precidence: str = "mult_div") -> int:
        """Finds the next operator in the expression that should be solved next,
        assuming multiplication and division first.
    ...
    # Gets the single operator expression we will be solving for from the overal
    # expression
    def get_subexpression(
        self, expression: str, pos_operator: int
    ) -> tuple[str, int, int]:
        ...
        if left == 1 and expression[0] == "-":
            left = 0  # include leading negative sign if needed
    ...
    def do_the_math(self, first_num: float, second_num: float, operator: str) -> float:
        ...
        # these are all lambda calls which can be found just above the function
        # definition for self.do_the_math
        match operator:
            case "+":
    ...
    def get_exponent_answer(self, base: float, exponent: float) -> float:
        ...
        # Reject negative base + non-integer exponent - creates complex numbers,
        # calculator designed for real numbers only
        if base < 0 and not exponent.is_integer():
            raise CalculatorError(
                (
                    "Error: Negative base with non-integer exponent results in complex"
    ...
    def which_operator(self, expression_to_calc: str) -> int:
        """Controls the precidence for which operator self.find_operator_pos should be
        looking for.
        ...
        if pos_operator == -1:
            pos_operator = self.find_operator_pos(
                expression_to_calc
            )  # no precidence flag so it assumes multiplicatoin and division
        if pos_operator == -1:
            pos_operator = self.find_operator_pos(expression_to_calc, "add_sub")

        return pos_operator

    def get_elements(
        self, expr: str, left: int, right: int, op: int
    ...
    def calculate(self, expr: str, is_paren_exp: bool) -> str:
        ...
        for i in range(num_operators):
            pos_operator = self.which_operator(
                expr
            )  # get the operator position to solve for next based on precidence PEMDAS

            # get the math problem we are going to solve along with that problems first
            # and last position in the over all expression
            sub_exp, left_num, right_num = self.get_subexpression(expr, pos_operator)

            first_num, second_num, operator = self.get_elements(
                expr, left_num, right_num, pos_operator
            )  # Gets the elements of the expression to solve for
    ...
    def get_do_another(self) -> str:
        """Prompts the user, asking if they would like to enter another expression.
        Defaults to yes.
    ...
    def is_balanced_paren(self, expr: str) -> tuple[bool, str]:
        """Verifies that the parenthesis are balanced and fixes single digit parenthesis
        like 4*(5) to 4*5 or (5)*4 to 5*4.
    ...
    def return_bal_paren_or_ambiguous(
        self, expr: str, check_paren: bool
    ) -> tuple[bool, str]:
        """Helper function for is_balanced_paren and is_ambiguous that returns if the
        parenthesis are balanced or if the expression could be ambiguous depending on
        ...
        for i, char in enumerate(expr):
            if char in pairs.values():  # opening: (, [, {
                ...
                # fix singledigit parenthesis like 4*(5), (5)*4, -(5)*4 and -(-5)*4 and
                # check for balanced paren again
    ...
    def is_ambiguous(self, expr: str) -> bool:
        """Checks to see if the expression is ambiguous. An expression is ambiguous if
        there are parenthesis that do not have an operator to the left or right of them.
        For example, 4(5+2) is ambiguous because it could be 4*(5+2) or 45+2. (5+2)4 is
        also ambiguous because it could be (5+2)*4 or 5+24.
    ...
    def fix_double_neg_at_beg_of_expr(
        self, expr: str, open_paren_pos: int
    ) -> tuple[str, bool]:
        """Checks the beginning of the expression for a negitive just outside the open
        parenthesis and a negative just inside the parenthesis. If this is the case, it
        fixes the expression by removing both negative signs. This simplifies the
        expressioin from a double negative to a positive and also allows us to avoid
        issues with double negatives when we drop parenthesis. This is the only case of
        single digit parenthesis that we need to worry about because any other case of
        single digit parenthesis would be caught by other checks that are done.
        ...
        ):  # check for negative sign outside and inside the parenthesis
            expr = "(" + expr[3:]
    ...
    def is_valid_input(self, input_to_validate: str) -> bool:
        """Checks to make sure that input is only 0-9 . ()+-*/[]{}^% also checks for
        matching parenthesis also checks to make sure that if there are parenthesis,
        that expression to the right and left of them start with an operator and not a
        number also checks for division by 0 also checks for double operators also
        checks for trailing or leading operators.
        ...
        # Check for balanced parenthesis and ambiguous parenthesis - also fixs single
        # digit paren
        ...
        if val:
            if any(char in input_to_validate for char in ["()", "[]", "{}"]):
                print(
                    "Empty brackets detected, they will be removed from the expression"
                )
    ...
    def has_double_op(self, expr: str) -> bool:
        """Checks the expression to see if there are double operators. Will ignore an
        expression that double negative signs, or negative signs after the operator.
    ...
    def has_leading_or_trailing_op(self, expr: str) -> bool:
        """Checks the expression to see if there are leading or trailing operators. Will
        ignore an expression that starts with a negative sign.
    ...
    def get_num_input(self) -> str:
        """Collects the input from the user, cleans it and calls the validation sequence
        on it passing any validation issues onto the user.
        ...
        """
        greeting = (
            "Please enter your expression (use '%' for MOD and '^' for exponents): "
        )
        val = False
    ...
    def main(self) -> None:
        """Main entry for the program calls get_num_input once that is return with valid
        input it calls pars_expression to do the rest of the work then it asks if the
        user wants to do another expression.
